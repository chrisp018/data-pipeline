...
module "zookeeper" {
  ...
  user_data_path          = "${path.root}/zookeeper_mgmt/zkp_config/zkp_install.tpl"
  provisioner_file_source = "${path.root}/zookeeper_mgmt/zkp_config"
  provisioner_file_dest   = "/home/ubuntu/mgmt"
  # remote_exec = {}
  local_exec_var = "zookeeper"
}

module "zookeeper_monitoring" {
  source                  = "./compute"
  resource_group_name     = "zkp_mntr"
  public_sg               = module.networking.public_sg
  public_subnets          = module.networking.public_subnets
  profile_name            = "${local.iam_role.instance_role.name}_profile_zkp_mntr"
  instance_role_name      = local.iam_role.instance_role.name
  instance_count          = 1
  instance_type           = "t2.micro"
  vol_size                = "10"
  ...
  private_key_path        = "${var.home_path}.ssh/bigdata_stream"
  key_name                = "bigdata_stream"
  user_data_path          = "${path.root}/zookeeper_mgmt/tools/tools.tpl"
  provisioner_file_source = "${path.root}/zookeeper_mgmt/tools"
  provisioner_file_dest   = "/home/ubuntu/mgmt"
...
module "kafka" {
  ...
  private_key_path        = "${var.home_path}.ssh/bigdata_stream"
  key_name                = "bigdata_stream"
  user_data_path          = "${path.root}/kafka_mgmt/kafka_config/kafka_install.tpl"
  provisioner_file_source = "${path.root}/kafka_mgmt/kafka_config"
  provisioner_file_dest   = "/home/ubuntu/mgmt"
